...
# 5. MATHEMATICAL MODEL COMPUTATIONS
...
# Build target market lists out cleanly based on user layout choices
...
# --- TAB 1: RENDER AND FILTER ENGINE ---
with main_tab:
    ...
    if options_pool:
        ...
        if st.button("🚀 Push Slip to Group Ledger"):
            data_type, match_meta = options_pool[target_selection]
            new_log = {
                "Friend": f_name, 
                "Type": "Game Line" if data_type == "game" else "Player Prop",
                "Matchup": match_meta["Matchup"], 
                "Selection": match_meta["Selection"], 
                "Odds": match_meta["Odds"],
                "Wager ($)": actual_wager, 
                "True Prob.": f"{match_meta['True Prob.']*100:.1f}%",
